...
function getRecentSavedData() {
    ...
    getServiceAreaCategory(areaId.split('_')[0]);
...
function showForm() {
    var y = document.getElementsByClassName("w2ui-panel-title");
    y[2].textContent = "PRODUCTION DATA - QUERY VIEW (" + clientName;
    var svc = $("#ddlServiceArea");
    serviceAreaId = parseInt(svc.val());
    serviceAreaName = svc.find("option:selected").text();
    if (isFiltered && clientName !== '') y[2].textContent = "PRODUCTION DATA - ENTRY VIEW (" + clientName + " | " + serviceAreaName;
    y[2].textContent += ")";
    y[4].textContent = "DETAILS (" + siteName + " | " + locName + " | " + serviceAreaName + ")";
    ...
    $("#gridContainer").dxDataGrid({
        ...
        onToolbarPreparing: function (e) {
            e.toolbarOptions.items.unshift({
                ...
                widget: "dxButton",
                options: {
                    visible: true,//isFiltered,
                    //icon: "material-icons ic-site",
                    text: "Set Site",
                    showText: "always",
                    icon: "site-icon",
                    hint: "Select New Site",
                    onClick: function () {
                        showSelectors();
                        setSelectors();
                        ajaxRequest("#ddlSite", baseUrl + "api/client/GetSiteByUserName", { Id: $("#ddlClient").val(), email: sessionEmail });
                ...
                options: {
                    visible: true,//isFiltered,
                    //icon: "material-icons ic-location",
                    text: "Set Location",
                    showText: "always",
                    icon: "location-icon",
                    hint: "Select New Location",
                    onClick: function () {
                        showSelectors();
                        setSelectors();
                        ajaxRequest("#ddlLocation", baseUrl + "api/client/GetLocationById", { Id: $("#ddlSite").val(), email: sessionEmail });
                ...
                options: {
                    visible: true,
                    //icon: "material-icons ic-area",
                    text: "Set Area",
                    showText: "always",
                    icon: "area1-icon",
                    hint: "Select New Service Area",
                    onClick: function () {
                        showSelectors();
                        setSelectors();
                        ajaxRequest("#ddlServiceArea", baseUrl + "api/client/GetServiceAreaById", { Id: $("#ddlLocation").val(), email: sessionEmail })
                ...
                options: {
                    ...
                    showText: "always",
                    icon: "fa fa-download",
                    hint: "Export All Transactions (For Upload Later)",
                    onClick: function () {
                        exportToCsv(true);
                    }
                }
        ...
        onCellDblClick: function (e) {
            ...
            if (!isFiltered) {
                ...
                if (savedSelectorsArr.length > 0) {
                    ...
                    else getSavedDataByServiceArea(areaId);
                }
            }
        },
        hoverStateEnabled: true,
...
function populateForms(data) {
    ...
    $.each(d_data, function (i, e) {
        ...
        if (e.IsMandatory == 1) isMandatory = true;
...
function generateCsvData(allVisible) {
    ...
    if (model.Transactions && model.Transactions.length > 0) {
        for (var i = 0; i < model.Transactions.length; i++) {
            ...
            model.Transactions[i].AccountName,
            model.Transactions[i].SiteName,
            model.Transactions[i].LocationName,
            model.Transactions[i].ServiceAreaName,
